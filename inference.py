...
def main(model_args, args):
    ...
    if args.data_dir is not None:
        ...
        pairs_csv_path = args.data_dir / 'pairs.csv'
        if not pairs_csv_path.exists():
            raise FileNotFoundError(f"pairs.csv not found in {args.data_dir}")
        ...
        for _, row in tqdm(df.iterrows(), total=len(df), desc="Processing pairs"):
            ...
            # Source is face, target is shape and color
            face_path = source_path
            shape_path = target_path
            color_path = target_path
            
            # Output path
            output_image_path = output_dir / f'{target_id}_to_{source_id}' / "transferred.png"
            ...
            exp_name = f'{target_id}_to_{source_id}' if model_args.save_all else None
            
            # Run inference
            final_image = hair_fast.swap(face_path, shape_path, color_path, 
                                        benchmark=args.benchmark, exp_name=exp_name)
        ...
        for exp in tqdm(experiments):
            ...
            if isinstance(exp, str) or args.result_path is None:
                os.makedirs(args.output_dir / base_name, exist_ok=True)
                output_image_path = args.output_dir / base_name / "transferred.png"
            else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='HairFast inference')
    
    # New argument for pairs.csv workflow
    parser.add_argument('--data_dir', type=Path, default="/workspace/celeba_subset", 
